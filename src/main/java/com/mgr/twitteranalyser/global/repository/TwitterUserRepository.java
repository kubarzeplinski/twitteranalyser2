...
public interface TwitterUserRepository extends PagingAndSortingRepository<TwitterUser, Long>, Serializable {
    ...
    @Query("MATCH (keyword:Keyword)<-[relations:RETWEETED_TO_RELATIONS|:INTERESTED_IN_RELATIONS*]-(user:TwitterUser) " +
    ...
    @Query("MATCH (keyword:Keyword)<-[relation:RETWEETED_TO_RELATIONS|:INTERESTED_IN_RELATIONS*]-(user:TwitterUser) " +
    ...
    @Query("MATCH (user:TwitterUser)<-[relation:INTERESTED_IN_RELATIONS]->(keyword:Keyword) " +
            "WHERE keyword.name = {keyword} " +
            "RETURN user")
